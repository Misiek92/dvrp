...
for ($i = 0; $i < LOOPS; $i++) {
    ...
    if (TASKS <= 6 && RESOURCES <= 3) {
        ...
        $naive->execute();
        $naive->getInfo();
        $percentageLongest = round(10000 * ((float) $scenarios[$algorithm][0] / (float) $naive->getTheLongestDistance())) / 100;
        $percentageTotal = round(10000 * ((float) $scenarios[$algorithm][1] / (float) $naive->totalDistance())) / 100;
        clearDistances($resources);
    }
    ...
    $file = file_put_contents('results/prefixes.txt', "\r\n" . $date->format('YmdHis') . "." . $unique . " " . $algorithms[$algorithm] . " (" . $percentageLongest . " %) (" . $percentageTotal . " %)", FILE_APPEND);
